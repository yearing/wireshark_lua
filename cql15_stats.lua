...
local cq15_stats = Proto("CQL15Stats", "CQL15 Statistics")
...
local S = {
    ...
    sig_to_iscs = 0,
    iscs_to_sig = 0,
    type_cnt   = {},   -- 消息类型计数 {["01"]=N, ...}
    crc_ok     = 0,
    crc_fail   = 0,
    seq_dup    = 0,    -- 重号
    seq_gap    = 0,    -- 断号
    seq_seen   = {},   -- 已见序号 key=port_dir
    first_ts   = nil,
    last_ts    = nil,
    hb_interval_sum = 0,
    hb_interval_cnt  = 0,
...
-- 判断方向：src_port 在监控端口列表中 → SIG发起
local sig_ports = {}
local function is_sig_direction(pinfo)
...
local function dir_label(pinfo)
    return is_sig_direction(pinfo) and "SIG→ISCS" or "ISCS→SIG"
end

local function seq_key(pinfo, is_sig)
    local d = is_sig and "S" or "I"
    return tostring(pinfo.src_port) .. d
end
...
local function update(buffer, pinfo, tree)
    ...
    local is_sig = is_sig_direction(pinfo)
    if is_sig then
    ...
    if buffer:len() >= 10 then
        ...
        if dtype == "09" then
            if S.hb_last_ts then
                ...
                S.hb_interval_sum = S.hb_interval_sum + iv
                S.hb_interval_cnt = S.hb_interval_cnt + 1
                if iv > S.hb_interval_max then S.hb_interval_max = iv end
                if not S.hb_interval_min or iv < S.hb_interval_min then
                    S.hb_interval_min = iv
                end
            end
            S.hb_last_ts = ts
        end
    ...
    -- CRC 统计（从 tree 查找 CRC 字段标注）
    -- 简化：统计 crc 字段是否存在 FAIL
    -- 实际 CRC 在 dissect_message 中已标注 OK/FAIL
end
...
local function stat_cb(dialog)
    ...
    for dtype, cnt in pairs(S.type_cnt) do
        local name = S.type_names and S.type_names[dtype] or dtype
...
-- 注册
-- ============================================================
cq15_stats.prefs.my_tcp_port = Pref.string("监控端口(s):", "5000",
    "CQL15默认端口，多个用逗号分隔")
...
local function add_port()
    sig_ports = {}
    local port_str = cq15_stats.prefs.my_tcp_port
    for port in string.gmatch(port_str, "%d+") do
        local p = tonumber(port)
        tcp_port:add(p, cq15_stats)
        sig_ports[p] = true
    end
end
add_port()
function cq15_stats.prefs_changed() add_port() end

-- StatDialog 注册（Wireshark 2.0+）
if StatDialog then
    StatDialog.register(cq15_stats, "CQL15 统计分析", stat_cb)
end

-- Post-dissector：拦截所有包，对 cql15 协议包做统计
local function post_cb(buffer, pinfo, tree)
    local proto_col = pinfo.cols.protocol
    if proto_col and proto_col:set and
       string.find(proto_col:set, "CQL15", 1, true) then
        update(buffer, pinfo, tree)
    end
end
register_postdissector(post_cb)
